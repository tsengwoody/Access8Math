...
在编辑区按下 alt+h 可转换出包含文字内容与数学内容的 HTML 文件并可进行预览或导出。其中 LaTeX 区域的内容汇转成 MathML 便于跟普通文字一起呈现。

*	预览：通过已设置的关联 .HTML 扩展名的程序打开转换后的 HTML 文件。
*	导出：将转换后的 HTML 文件打包成一个压缩文件。

在编辑区按下 alt+m 弹出标记命令窗口，选择 LaTeX 按下 enter 键，即可在当前所选择的文字前后（未选择文字时在光标处）插入 LaTeX 标记并会自动将光标移入其中，以快速输入内容。

在编辑区按下 alt+l 弹出 LaTeX 命令窗口，选择要加入的 LaTeX 命令项目按下 enter 键，即可在当前光标处加入对应的 LaTeX 语法并会自动将光标移动到适当的插入位置，以快速输入内容。
...
alt+h, alt+i, alt+l, alt+m 为书写手势群组，在编辑区中按 alt+w 可启用或停用书写手势。
...
*	在编辑区按下 alt+shift+向下键不移动仅选取当前数据区域
*	在编辑区按下 alt+shift+向右键移动到下一个数据区域并选取

在编辑区按下 alt+s 启用或停用快捷手势。当启用快捷手势时按 f1~f12 可快速插入  LaTeX 语法。当启用快捷手势时按 shift+f1~f12 可读出该快捷键下绑定的 LaTeX 指令。
...
*	「常规设置」对话框：
	...
	*	使用 NVDA+alt+字母来开关指令手势：在编辑区的书写手势、区块导航手势、快捷手势开关时是否需加上 NVDA 键
*	「规则设置」对话框：选择特定规则是否启用。
