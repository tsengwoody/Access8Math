...
*	在任意編輯區按下 alt+向右鍵彈出標記指令視窗，選擇 LaTeX 按下 enter 鍵即會在當前游標處加入 LaTeX 標記區並會自動將游標移入其內，以快速輸入內容。
*	在任意編輯區按下 alt+向左鍵彈出 LaTeX 指令視窗，選擇 要加入的 LaTeX 指令項目按下 enter 鍵即會在當前游標處加入對應的 LaTeX 語法並會自動將游標移入適當輸入點，以快速輸入內容。
