...
*	以 LaTeX/AsciiMath 書寫數學內容
*	指令選單輸入 LaTeX/AsciiMath 語法
*	快捷手勢指令輸入 LaTeX/AsciiMath 語法
*	書寫導航游標移動編輯
*	檢視匯出 HTML 文件
...
此 NVDA 附加元件提供數學內容的書寫，透過指令選單，使得在輸入 LaTeX/ AsciiMath 語法時無需記憶即可完成輸入任務，在完成撰寫內容後，可透過檢視匯出功能將文件製作成可視讀之 HTML 文件，透過 HTML 文件可便利的與其他人進行互動討論。
...
互動：在數學物件上按下空白或 enter 可與該數學內容進行互動導航，亦即可部份瀏覽數學內容中的子內容、在內容間移動、縮放檢閱內容片段之大小
...
*	一般設定中「當進入互動模式時，顯示 Access8Math 互動視窗」勾選時，會開啟「Access8Math 互動視窗」，當系統焦點移至互動按鈕時會自動進入互動導航：
	*	互動：進入互動導航
	*	複製：複製數學物件之 MathML 碼
*	一般設定中「當進入互動模式時，顯示 Access8Math 互動視窗」無勾選時，則會直接進入互動導航功能：
	*	上下左右鍵可在內容間移動、縮放檢閱內容片段之大小
	數字鍵盤 1~9 可逐字、逐行閱讀序列化文字的數學內容
...
### 書寫綜合內容（已下快速鍵功能在編輯區可用，建議使用記事本）

以 LaTeX 或 AsciiMath 分隔標記框出數學標記區，用以區分文字內容與數學內容，亦即在數學標記區內的資料為 LaTeX/AsciiMath 語法的數學內容，在數學標記區外的為文字內容。

*	LaTeX 分隔標記：（可於一般設定中選擇使用括號或錢號）
	*	開始分隔標記：「\(」 或 「$」
	*	結束分隔標記：「\)」 或 「$」
*	AsciiMath 分隔標記：
	*	開始分隔標記：「`」
...
alt+l：彈出 LaTeX 指令視窗，選擇要加入的 LaTeX 指令項目按下 enter 鍵即會在當前游標處加入對應的 LaTeX 語法並會自動將游標移入適當輸入點，以快速輸入內容。
...
alt+h：可轉換出文字資料與數學資料的 HTML 文件並可進行檢視或匯出。其中數學區塊之內容匯轉成 MathML 並與文字並列呈現，以利視覺閱讀。

*	檢視：透過系統設定的預設開啟 .HTML 副檔名應用程式打開轉換後的 HTML 文件。
*	匯出：將轉換後的 HTML 文件與原始文字檔打包儲存為一壓縮檔。

alt+t：可將游標所在的區塊資料在 LaTeX 與 AsciiMath 間轉換（游標需在 LaTeX 或 AsciiMath 區塊內）

在一般設定內可選擇啟動時是否啟用指令手勢，編輯區中按 NVDA+alt+c 可啟用或停用指令手勢，可於輸入手勢中變更。
...
在一般設定內可選擇啟動時是否啟用區塊導航手勢，編輯區中按 NVDA+alt+n 可啟用或停用區塊導航手勢，可於輸入手勢中變更。

### 快捷手勢（開關：NVDA+alt+s）

當游標在 LaTeX 區塊時，按字母、 f1~f12 可快速插入  LaTeX 語法。按 shift+字母、shift+f1~f12 可讀出該快捷當下綁定的 LaTeX 語法。（需先於指令選單中設定快捷鍵）

在一般設定內可選擇啟動時是否啟用快捷導航手勢，編輯區中按 NVDA+alt+n 可啟用或停用快捷導航手勢，可於輸入手勢中變更。

### 書寫導航模式（開關：NVDA+space）

當書寫導航模式開啟時，游標移動報讀出的數學資料區塊會讀出數學文字化內容而非原始語法資料

可用以下按鍵手勢移動編輯游標與互動導航

*	向左鍵：移動到上一個資料區塊的開始點並讀出
*	向右鍵：移動到下一個資料區塊的開始點並讀出
*	向上鍵：往上移動十行並讀出該行所有資料區塊的內容
*	向下鍵：往下移動十行並讀出該行所有資料區塊的內容
...
*	LaTeX（使用括號分隔符）：一元二次方程式 \(ax^2+bx+c=0\) 的解為 \(x=\frac{-b\pm\sqrt{b^2-4ac}}{2a}\) 。
*	LaTeX（使用錢號分隔符）：一元二次方程式 $ax^2+bx+c=0$ 的解為 $x=\frac{-b\pm\sqrt{b^2-4ac}}{2a}$ 。
...
*	新增書寫導航模式編輯快速鍵 - 剪下(ctrl+x)、複製(ctrl+c)、貼上(ctrl+v)、刪除(delete/back space)
*	新增書寫導航模式移動快速鍵 - 在可互動之資料區塊間移動(tab)、在 AsciiMath 資料區塊間移動(a)
*	調整書寫導航模式移動快速鍵 - 上下左右鍵移動游標並讀出移動後資料區塊內容
*	書寫導航模式游標移動時，數學資料區塊會讀出數學文字化內容而非原始語法
*	書寫導航模式游標停在數學資料區塊上時可按下 space 或 enter 鍵與該數學資料區塊互動導航
*	新增英文字母為可自訂之快捷按鍵
*	新增希臘字母快捷模式
*	快捷按鍵輸入僅在數學區有效
*	可設定以音效或語音提示書寫導航模式的切換
*	LaTeX 指令選單在文字區時亦可開啟並在插入時加上 LaTeX 分隔符
*	新增轉譯選單，可將游標所停留在的區塊 LaTeX/AsciiMath 資料格式互轉。屬於命令手勢組，當游標停留在 LaTeX/AsciiMath 區塊時，按下 alt+t 可開啟轉譯選單（在書寫導航模式下為 ctrl+t）
