...
*	在任意编辑区按下 NVDA+shift+m 可转换出混合文字与数学数据的 HTML 文件，其中 LaTeX 标记区之数据汇转成 MathML 以利与一般文字并列呈现。
*	HTML 文件可进行检视或汇出。
	*	检视：透过预设开启 .HTML 扩展名的程序打开转换后的 HTML 文件。
	*	汇出：将转换后的 HTML 文件打包成一压缩文件。
*	范例：一元二次方程式\(ax^2+bx+c=0\)的解为 \(\frac{-b\pm\sqrt{b^2-4ac}}{2a}\)。
*	在任意编辑区按下 NVDA+向右键弹出标记指令窗口，选择 LaTeX 按下 enter 键即会在当前光标处加入 LaTeX 标记区并会自动将光标移入其内，以快速输入内容。
*	在任意编辑区按下 NVDA+向左键弹出 LaTeX 指令窗口，选择 要加入的 LaTeX 指令项目按下 enter 键即会在当前光标处加入对应的 LaTeX 语法并会自动将光标移入适当输入点，以快速输入内容。
