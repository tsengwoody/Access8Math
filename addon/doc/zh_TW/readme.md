...
Access8Math 提供 MathML 內容的語音與點字支援。它也提供互動模式，讓使用者可以用較小的片段探索數學內容，並更容易理解其結構與階層關係。

Access8Math 也可以協助撰寫 LaTeX 並轉換為 MathML。LaTeX 是一種易於撰寫與學習的排版系統，常用於表示數學公式。Access8Math 也提供指令選單、預覽支援與匯出功能，讓數學編輯與分享更加實用。

透過對無障礙數學閱讀與高效率數學書寫的支援，Access8Math 讓視障使用者與明眼使用者之間的數學溝通更加容易。
...
* Alt + H：顯示用於預覽或匯出的檢視指令選單（虛擬選單）。詳細資訊請參閱「匯入與匯出」章節。
...
Access8Math 編輯器的檢視選單內的匯出功能讓使用者可以保存與分享文件。使用者選擇匯出路徑與檔名後，匯出功能將以相同檔名輸出兩種檔案：
...
例如匯出路徑為 `D:\`，檔名為 `test`，Access8Math 將產生 `D:\test.a8m` 與 `D:\test.zip`。使用者可以解壓縮 `test.zip` 並開啟 `index.html` 來檢視文件，HTML 文件中的數學內容將以 MathML 顯示。
...
較早版本的更新內容請參閱 [changelog.md](changelog.md)。
