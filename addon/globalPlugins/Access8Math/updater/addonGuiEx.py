...
def onAddonUpdateCheck(evt):
	...
	_progressDialog = gui.IndeterminateProgressDialog(
		...
		_("Checking for Access8Math update...")
	)
...
class AddonUpdatesDialog(wx.Dialog):
	...
	def __init__(self, parent, addonUpdateInfo, auto=True):
		self.addon_name = "Access8Math"
		self.auto = auto
		addonUpdateInfo = addonUpdateInfo if addonUpdateInfo else {}
		self.addonUpdateInfo = {}
		for key, value in addonUpdateInfo.items():
			if self.addon_name != key:
				continue
			self.addonUpdateInfo[key] = value
		if len(self.addonUpdateInfo) > 1:
			raise ValueError("addonUpdateInfo count incorrect")
		self.version = self.addonUpdateInfo[self.addon_name]["version"]

		# Translators: The title of the add-on updates dialog.
		super().__init__(
			parent,
			title=_("{addon_name} Update").format(addon_name=self.addon_name)
		)
		mainSizer = wx.BoxSizer(wx.VERTICAL)
		addonsSizerHelper = guiHelper.BoxSizerHelper(self, orientation=wx.VERTICAL)
		if len(self.addonUpdateInfo) == 1:
			# Translators: Message displayed when add-on update is available.
			addonsSizerHelper.addItem(wx.StaticText(
				self,
				label=_("{addon_name} has released new version {version}").format(
					addon_name=self.addon_name,
					version=self.version
				)
			))
		else:
			# Translators: Message displayed when no add-on update is available.
			addonsSizerHelper.addItem(wx.StaticText(
				self,
				label=_("{addon_name} is the latest version").format(addon_name=self.addon_name)
			))
	...
	def onUpdate(self, evt):
		...
		next(updateAddonsGenerator([list(self.addonUpdateInfo.values())[0]], auto=self.auto))
