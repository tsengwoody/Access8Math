...
此 NVDA 附加组件提供数学内容的书写，透过指令选单，使得在输入 LaTeX/ AsciiMath 语法时无需记忆即可完成输入任务，在完成撰写内容后，可透过检视指令将文件制作成可视读之 HTML 文件，透过 HTML 文件可便利的与其他人进行互动讨论。

## 阅读功能

报读：可以串行化文字报读在网页浏览器（Mozilla Firefox, Microsoft Edge and Google Chrome）上以 MathML 撰写的数学对象或在 Microsoft Word 上以 MathType 撰写的数学对象（需安装 MathType）
...
* 阅读设定中「当进入互动导航模式时，显示互动窗口」勾选会开启「Access8Math 互动窗口」，当系统焦点移至互动按钮时会自动进入互动导航

    * 互动：进入互动导航
    * 复制：复制数学对象之 MathML 码

* 阅读设定中「当进入互动模式时，显示 Access8Math 互动窗口」无勾选则会直接进入互动导航功能
...
* home键：回到最顶层（完整数学内容）
* 数字键盘1-9：使用NVDA 检阅模式阅读数学内容（请参见 NVDA 用户指南的检阅模式章节）
* esc键：退出互动导航模式
...
### 书写综合内容（快捷键可在编辑区使用，建议使用附加组件内之内建编辑器。windows 11 必需使用附加组件所提供之编辑器；windows 10 以前版本可使用记事本。）

以 LaTeX/AsciiMath 分隔标记框出数学标记区，用以区分文字内容与数学内容，亦即在数学标记区内的数据为 LaTeX/AsciiMath 语法的数学内容，在数学标记区外的为文字内容。
...
alt+l：弹出 LaTeX 指令窗口，选择要加入的 LaTeX 指令项目按下 enter 键即会在当前光标处加入对应的 LaTeX 语法并会自动将光标移入适当输入点，以快速输入内容（如光标未在 LaTeX 区内会自动加上开始与结束标记）。

LaTeX 指令窗口

* 选到任意 LaTeX 指令项目按下英文字母(a~z)或 f1~f12 设定快捷
* 选到任意 LaTeX 指令项目按下 delete/back space 移除已设定的快捷
* 选到任意 LaTeX 指令项目按下 enter 在当前光标处加入对应的 LaTeX 语法

alt+i：光标停在数学区块上时，可与该数学区块进行互动导航

alt+h：可转换出文本块与数学区块的 HTML 文件并可进行预览或汇出。其中数学区块之内容汇转成 MathML 并与文字并列呈现，以利视觉阅读。

* 在书写设定中的 HTML 文件显示，可选择导出后的 HTML 文件以 Markdown 或纯文本解析转换。
* 在书写设定中的 HTML 数学显示，可选择导出后的 HTML 文件中数学对象是否为一区块。效果为在浏览模式上下箭头键移动报读整行内容时，数学对象是否独立一行呈现。
* 预览：透过系统设定的默认开启 .HTML 扩展名应用程序打开转换后的 HTML 文件。
* 汇出：将转换后的 HTML 文件与原始文本文件打包储存为一压缩文件，压缩文件内包含 HTML (可视读、听读、摸读格式)与 txt (原始可编辑档案)

alt+t：可将游标所在的数学区块在 LaTeX 与 AsciiMath 间转换（游标需在数学区块内）

alt+b:可将整份文件进行批次转换，包括 LaTeX 分隔符括号与钱号互转、LaTeX 与 AsciiMath 互转。
...
当光标在 LaTeX 区块时，按字母、 f1~f12 可快速插入绑定之 LaTeX 语法。按 shift+字母、shift+f1~f12 可读出该快捷当下绑定之 LaTeX 语法。（需先于指令选单中设定快捷键）

在书写设定内可选择启动时是否启用快捷手势，编辑区中按 NVDA+alt+s 可启用或停用快捷手势，可于输入手势中变更。

### 希腊字母手势（开关：NVDA+alt+g）

当光标在 LaTeX 区块时，按字母可快速插入对应的希腊字母 LaTeX 语法。
...
当浏览导航模式开启时，光标移动报读出的数学区块会读出数学文字化内容而非原始语法
...
下列的按键，若仅按该单一键，编辑游标会跳至其对应的后一个数据区块位置，若同时按 shift + 该单一按键，编辑光标会跳至前一个数据区块位置：
...
windows 11 起记事本的编辑区为丰富文本区域非传统编辑区，故在 windows 11 必需使用此编辑器方能使用书写功能。此编辑器亦提供开启旧档、储存档案、预览等功能。

检视菜单下的预览、导出功能与指令手势下的检视功能（alt+h）相同可产生 HTML 檔。

在编辑器下产生的 HTML 档会包括原始 txt 文件所在的文件夹内数据，可使 Markdown 语法有关联资源数据时可正确显示。

### 综合内容范例：

* LaTeX（使用括号分隔符）：一元二次方程式 \(ax^2+bx+c=0\) 的解为 \(x=\frac{-b\pm\sqrt{b^2-4ac}}{2a}\) 。
* LaTeX（使用钱号分隔符）：一元二次方程式 $ax^2+bx+c=0$ 的解为 $x=\frac{-b\pm\sqrt{b^2-4ac}}{2a}$ 。
...
* 语音、点字、互动来源设定选项移到偏号->设定->数学阅读器内
* 整合 MathCAT，当已安装了 Math Player/MathCat 时，您可以在数学阅读器设定中选择您需要的语音、点字和交互来源（Access8Math/Math Player/MathCAT）。
* 使用 MultiCategorySettingsDialog 搜集设定对话框。
* 按 NVDA+alt+e 在档案总管中使用内建编辑器打开文字文件。
* 在虚拟选单中的子菜单可透过 enter 展开
* 实做 MathML menclose tag 规则
