...
*	在任意编辑区按下 alt+向右键弹出标记指令窗口，选择 LaTeX 按下 enter 键即会在当前光标处加入 LaTeX 标记区并会自动将光标移入其内，以快速输入内容。
*	在任意编辑区按下 alt+向左键弹出 LaTeX 指令窗口，选择 要加入的 LaTeX 指令项目按下 enter 键即会在当前光标处加入对应的 LaTeX 语法并会自动将光标移入适当输入点，以快速输入内容。
