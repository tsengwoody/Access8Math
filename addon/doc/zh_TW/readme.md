...
*	使用開始分隔標記「\(」與結束分隔標記「\)」來判斷文字內容與數學內容的區域，亦即在開始分隔標記與結束分隔標記內的資料為數學內容(LaTeX)，在分隔標記外的為文字內容。
*	轉換為 HTML 格式
	*	功能表中的 「text-math...」 會顯示一編輯區輸入綜合內容後按下確定
	*	在任意編輯區按下 NVDA+shift+m
*	轉換完成後會開啟「Access8Math HTML 視窗」
	*	檢視：透過預設開啟 HTML 檔案的程式打開轉換完成的結果
	*	匯出：將轉換完成的檔案打包成一壓縮檔
