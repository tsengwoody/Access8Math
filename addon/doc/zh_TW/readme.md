...
*	在任意編輯區按下 NVDA+shift+m 可轉換出混合文字與數學資料的 HTML 文件，其中 LaTeX 標記區之資料匯轉成 MathML 以利與一般文字並列呈現。
*	HTML 文件可進行檢視或匯出。
	*	檢視：透過預設開啟 .HTML 副檔名的程式打開轉換後的 HTML 文件。
	*	匯出：將轉換後的 HTML 文件打包成一壓縮檔。
*	範例：一元二次方程式\(ax^2+bx+c=0\)的解為 \(\frac{-b\pm\sqrt{b^2-4ac}}{2a}\)。
*	在任意編輯區按下 NVDA+向右鍵彈出標記指令視窗，選擇 LaTeX 按下 enter 鍵即會在當前游標處加入 LaTeX 標記區並會自動將游標移入其內，以快速輸入內容。
*	在任意編輯區按下 NVDA+向左鍵彈出 LaTeX 指令視窗，選擇 要加入的 LaTeX 指令項目按下 enter 鍵即會在當前游標處加入對應的 LaTeX 語法並會自動將游標移入適當輸入點，以快速輸入內容。
